...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._missing_cols_logged: set = set()
    ...
    def _log_missing_column_once(self, name: str) -> None:
        """Log missing optional columns once and leave their tensors as zeros.

        The official evaluation data should contain all schema columns. This
        fallback is mainly for small demo parquet files whose schema can be
        incomplete; zero-filling keeps smoke tests runnable.
        """
        if name not in self._missing_cols_logged:
            logging.warning(f"Column {name} missing from parquet; filled with zeros")
            self._missing_cols_logged.add(name)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for ci, dim, offset, vs in self._user_int_plan:
            if ci is None:
                self._log_missing_column_once(f'user_int@offset={offset}')
                continue
            col = batch.column(ci)
            if dim == 1:
        ...
        for ci, dim, offset, vs in self._item_int_plan:
            if ci is None:
                self._log_missing_column_once(f'item_int@offset={offset}')
                continue
            col = batch.column(ci)
            if dim == 1:
                arr = col.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
        ...
        for ci, dim, offset in self._user_dense_plan:
            if ci is None:
                self._log_missing_column_once(f'user_dense@offset={offset}')
                continue
            col = batch.column(ci)
        ...
        for domain in self.seq_domains:
            ...
            for ci, slot, vs in side_plan:
                if ci is None:
                    self._log_missing_column_once(f'{domain}_seq_slot={slot}')
                    continue
                col = batch.column(ci)
