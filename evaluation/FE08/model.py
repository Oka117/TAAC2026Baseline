...
class TokenGNNLayer(nn.Module):
    """One lightweight complete-graph message-passing layer over NS tokens."""

    def __init__(
        self,
        d_model: int,
        dropout: float = 0.0,
        layer_scale_init: float = 0.15,
    ) -> None:
        super().__init__()
        self.norm = nn.LayerNorm(d_model)
...
class TokenGNN(nn.Module):
    ...
    def __init__(
        ...
        if graph_type != 'full':
            raise ValueError(
                f"Unsupported TokenGNN graph_type={graph_type}; only 'full' is supported")
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        token_gnn_layer_scale: float = 0.15,
    ) -> None:
        super().__init__()
    ...
    def _build_ns_tokens(self, inputs: ModelInput) -> torch.Tensor:
        """Build non-sequential tokens and optionally run TokenGNN."""
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1. NS tokens: tokenizer output followed by optional TokenGNN.
