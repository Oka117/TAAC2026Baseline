...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Compatibility-only legacy flags. FE-07 keeps the NS token path
        # structure-free and rejects any checkpoint that enables them.
        ...
        if use_token_gnn:
            raise ValueError(
                "FE-07 evaluation is a no-GNN path; use_token_gnn must be false.")
        self.use_token_gnn = False
    ...
    def _build_ns_tokens(self, inputs: ModelInput) -> torch.Tensor:
        """Build non-sequential tokens for the FE-07 no-GNN path."""
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1. NS tokens: tokenizer output only.
