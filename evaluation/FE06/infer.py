...
from build_fe06_p0ab_dataset import (
...
def _resolve_fe06_sidecar(model_dir: str, filename: str) -> Optional[str]:
    local_path = os.path.join(model_dir, filename)
    if os.path.exists(local_path):
        return local_path
    stats_dir = os.environ.get('FE06_STATS_DIR') or os.environ.get('FE06_STATS_PATH')
    if stats_dir:
        candidate = os.path.join(stats_dir, filename)
        if os.path.exists(candidate):
            return candidate
...
def _load_fe06_stats(model_dir: str) -> Optional[Dict[str, Any]]:
    stats_path = _resolve_fe06_sidecar(model_dir, 'fe06_transform_stats.json')
    if stats_path is None:
        return None
    logging.info(f"Using FE-06 transform stats: {stats_path}")
    return _load_json(stats_path)


def _schema_has_fe06_columns(schema_path: str) -> bool:
    schema = _load_json(schema_path)
...
def _normalize_fe06_array(
    name: str,
    values: np.ndarray,
    dense_stats: Dict[str, Dict[str, float]],
) -> np.ndarray:
    if name not in dense_stats:
        raise KeyError(f"fe06_transform_stats.json missing dense_stats.{name}")
    stat = dense_stats[name]
    mean = float(stat.get('mean', 0.0))
    std = max(float(stat.get('std', 1.0)), 1e-6)
    return ((values.astype(np.float32) - mean) / std).astype(np.float32)
...
def _normalize_fe06_dense_column(
    col: pa.Array,
    name: str,
    dense_stats: Dict[str, Dict[str, float]],
) -> pa.Array:
...
def maybe_build_fe06_eval_dataset(
    data_dir: str,
    schema_path: str,
    model_dir: str,
    batch_size: int,
    ...
    vocab_shift_cols = set(str(k) for k in stats.get('vocab_shift_audit', {}).keys())

    missing_base = [name for name in (match_col, match_ts_col, 'item_int_feats_9', 'timestamp') if name not in first_names]
    if missing_base:
        raise KeyError("FE-06 eval transform missing required raw columns: " + ", ".join(missing_base))

    output_dir = os.environ.get('FE06_EVAL_DATA_DIR')
    if not output_dir:
        output_dir = tempfile.mkdtemp(prefix='taac_fe06_eval_')
    os.makedirs(output_dir, exist_ok=True)
    logging.info(f"Writing FE-06 eval parquet to: {output_dir}")
    logging.info(
        "FE-06 eval transform ignores eval label_type; total-frequency state "
    ...
    try:
        for input_path, batch in _iter_batches(files, batch_size):
            ...
            for name in vocab_shift_cols:
                if name in idx:
                    is_sequence = name.startswith('domain_') or name.startswith('seq_')
                    table = _fe06_append_or_replace_column(
                        table, name, _fe06_shift_sparse_column(batch.column(idx[name]), is_sequence=is_sequence))

            for name in dense_stats:
                if name in idx and name not in expected_generated:
                    table = _fe06_append_or_replace_column(
                        table, name, _normalize_fe06_dense_column(batch.column(idx[name]), name, dense_stats))

            feats = _fe06_compute_generated_features(
                batch,
                state,
                match_col,
                match_ts_col,
            ...
            for name in [
                ...
                values = _normalize_fe06_array(name, feats[name], dense_stats)
                if values.ndim == 1:
                    arr = pa.array(values, type=pa.float32())
                else:
                    arr = pa.array(values.tolist(), type=pa.list_(pa.float32()))
                table = _fe06_append_or_replace_column(table, name, arr)
...
def main() -> None:
    ...
    # ---- FE-06 transform-only preprocessing for raw eval data ----
    # FE-06 checkpoints expect sparse vocab-shifted columns plus generated
    # P0/FE01AB columns. Materialize them before dataset.py reads schema.json.
    data_dir = maybe_build_fe06_eval_dataset(
        data_dir=data_dir,
