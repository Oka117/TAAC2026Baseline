...
# FE-01 experiment entrypoint.
#
# Path policy:
# - Original dataset is read from TRAIN_DATA_PATH or --data_dir.
# - Augmented FE-01 parquet/schema are written outside the Training Code folder
#   by default, avoiding upload-code storage limits and read-only code paths.
# - train.py is then pointed at the generated FE-01 dataset.
...
FE01_ROOT="${FE01_DATA_DIR:-${TMPDIR:-/tmp}/taac_fe01_$$}"
FE01_SCHEMA="${FE01_ROOT}/schema.json"
FE01_GROUPS="${FE01_ROOT}/ns_groups.feature_engineering.json"

echo "[FE-01] input data: ${ORIG_DATA_DIR}"
echo "[FE-01] input schema: ${ORIG_SCHEMA_PATH}"
echo "[FE-01] output data: ${FE01_ROOT}"

python3 -u "${SCRIPT_DIR}/build_feature_engineering_dataset.py" \
    --input_dir "${ORIG_DATA_DIR}" \
    --input_schema "${ORIG_SCHEMA_PATH}" \
    --output_dir "${FE01_ROOT}" \
    --match_window_days 7 \
    --match_count_buckets 0,1,2,4,8 \
    --fit_stats_row_group_ratio 0.9

TRAIN_DATA_PATH="${FE01_ROOT}" python3 -u "${SCRIPT_DIR}/train.py" \
    "$@" \
    --schema_path "${FE01_SCHEMA}" \
    --ns_groups_json "${FE01_GROUPS}" \
