...
# 11thMay-PlanB temporal bucket features (12 dense, fid 113..124).
# For each domain ∈ {a, b, c, d} compute log1p(count of historical events
# matching the current sample's (hour_bucket, dow, month)). Vectorized numpy.
TEMPORAL_USER_DENSE_ADDS = [(fid, 1) for fid in range(113, 125)]
TEMPORAL_DENSE_FEATURE_NAMES = [f"user_dense_feats_{fid}" for fid in range(113, 125)]
TEMPORAL_DOMAINS = ("domain_a", "domain_b", "domain_c", "domain_d")
TEMPORAL_BUCKET_TYPES = ("hour", "dow", "month")  # 4 domains × 3 types = 12
...
FEATURE_SET_TO_EXPERIMENT = {
    ...
    "fe01a": "FE-01A",
    "fe01b": "FE-01B",
    "11thmay_a": "11thMay-PlanB",  # FE-01A frequency + 12 temporal buckets
}
...
def selected_user_dense_adds(feature_set: str, enable_delay_history: bool) -> List[Tuple[int, int]]:
    ...
    elif feature_set == "11thmay_a":
        # FE-01A frequency (110) + 12 temporal buckets (113..124)
        adds = [(110, 1)] + list(TEMPORAL_USER_DENSE_ADDS)
...
def selected_item_dense_adds(feature_set: str, enable_delay_history: bool) -> List[Tuple[int, int]]:
    if feature_set == "fe01a":
        ...
        adds = [(86, 1)]  # FE-01A baseline; no match dense features
    elif feature_set == "fe01b":
        adds = [(91, 1), (92, 1)]
    else:
...
def selected_item_int_adds(feature_set: str, match_count_vocab_size: int) -> List[Tuple[int, int, int]]:
    if feature_set in ("fe01a", "11thmay_a"):
        return []
...
# ---- 11thMay-PlanB: per-domain temporal bucket helpers ----

# Default ts_fid fallbacks per domain (used when schema doesn't specify).
# Verified for domain_d=26 (worktree FE-01B). Other domains use the highest
# fid in each domain's seq_* group as a heuristic; if the resolved column is
# absent in the parquet, the corresponding 3 features safely degrade to 0.
...
def resolve_domain_ts_columns(
    schema: Dict[str, Any], parquet_names: Sequence[str]
) -> Dict[str, Optional[str]]:
    """Return ``{domain: timestamp_column_name or None}`` for all 4 domains.

    Schema `seq[<domain>].ts_fid` is authoritative; fall back to
    `_DOMAIN_TS_FID_FALLBACK` heuristics when missing. Returns None for any
    domain whose resolved column is absent in the parquet, so downstream
    bucket-match counts safely degrade to 0 for that domain.
    """
    names = set(parquet_names)
    seq_cfg = schema.get("seq", {})
    ...
    for domain in TEMPORAL_DOMAINS:
        ...
        prefix = _strip_prefix(str(cfg.get("prefix", f"{domain}_seq")))
        ts_fid = cfg.get("ts_fid")
        if ts_fid is None:
            ts_fid = _DOMAIN_TS_FID_FALLBACK.get(domain)
        col = f"{prefix}_{ts_fid}" if ts_fid is not None else None
        out[domain] = col if (col is not None and col in names) else None
    return out


def _hour_bucket_from_unix(ts_sec: np.ndarray) -> np.ndarray:
    """Vectorized 3-bucket hour-of-day (UTC). Output dtype int8 in {0, 1, 2}.

    Buckets: 0=morning [6,12), 1=afternoon [12,18), 2=evening [18,6)
    Invalid (ts <= 0) returns -1 sentinel.
    """
    hour = ((ts_sec.astype(np.int64) // 3600) % 24).astype(np.int8)
    bucket = np.where((hour >= 6) & (hour < 12), 0,
              np.where((hour >= 12) & (hour < 18), 1, 2)).astype(np.int8)
    bucket = np.where(ts_sec > 0, bucket, np.int8(-1))
    return bucket


def _dow_from_unix(ts_sec: np.ndarray) -> np.ndarray:
    """Vectorized day-of-week (UTC). Output int8 in {0..6}, Monday=0.

    1970-01-01 (epoch day 0) was Thursday; (epoch_day + 3) % 7 makes Monday=0.
    Invalid (ts <= 0) returns -1 sentinel.
    """
    epoch_day = ts_sec.astype(np.int64) // 86400
    dow = ((epoch_day + 3) % 7).astype(np.int8)
    dow = np.where(ts_sec > 0, dow, np.int8(-1))
    return dow


def _month_from_unix(ts_sec: np.ndarray) -> np.ndarray:
    """Vectorized month (UTC). Output int8 in {0..11}, January=0.

    Uses numpy datetime64 for calendar math (vectorized over the array).
    Invalid (ts <= 0) returns -1 sentinel.
    """
    ts64 = ts_sec.astype(np.int64)
    dt = (ts64 * np.int64(1_000_000_000)).astype('datetime64[ns]')
    month = (dt.astype('datetime64[M]').astype(np.int32) % 12).astype(np.int8)
    month = np.where(ts_sec > 0, month, np.int8(-1))
    return month


def _list_ts_to_padded(
    arr: pa.Array, max_len: int, batch_size: int
) -> np.ndarray:
    """Pad a ListArray<int64> of event timestamps to shape (B, max_len). Pads with 0."""
    offsets = arr.offsets.to_numpy()
    values = arr.values.to_numpy().astype(np.int64, copy=False)
    ...
    for i in range(batch_size):
        ...
        if rl <= 0:
            continue
        ul = min(rl, max_len)
        out[i, :ul] = values[s:s + ul]
    return out


def _compute_temporal_bucket_features(
    batch: pa.RecordBatch,
    domain_ts_cols: Dict[str, Optional[str]],
    max_len: int = 1024,
) -> Dict[str, np.ndarray]:
    """Return 12 raw counts (not yet log1p / z-scored).

    Output keys: ``user_dense_feats_113..124`` (mapping):
      113-116: hour-bucket match count for domain_a/b/c/d
      117-120: dow match count for domain_a/b/c/d
      121-124: month match count for domain_a/b/c/d
    """
    ...
    # Per-sample bucket scalars (B,) — UTC.
    s_hb = _hour_bucket_from_unix(timestamps)
    s_dw = _dow_from_unix(timestamps)
    s_mo = _month_from_unix(timestamps)

    out: Dict[str, np.ndarray] = {}
    base_user_fid = 113
    for d_idx, domain in enumerate(TEMPORAL_DOMAINS):
        ts_col = domain_ts_cols.get(domain)
        # 3 fids per domain (hour, dow, month):
        #   hour:  113+d_idx
        #   dow:   117+d_idx
        #   month: 121+d_idx
        fid_hour = base_user_fid + d_idx
        fid_dow = base_user_fid + 4 + d_idx
        fid_month = base_user_fid + 8 + d_idx
        if ts_col is None or ts_col not in idx:
            zeros = np.zeros(B, dtype=np.float32)
            out[f"user_dense_feats_{fid_hour}"] = zeros
            out[f"user_dense_feats_{fid_dow}"] = zeros.copy()
            out[f"user_dense_feats_{fid_month}"] = zeros.copy()
            continue

        ev_ts = _list_ts_to_padded(batch.column(idx[ts_col]), max_len, B)
        # Defensive: only count events with ts > 0 AND ts <= sample timestamp.
        valid = (ev_ts > 0) & (ev_ts <= timestamps.reshape(-1, 1))
        ev_h = _hour_bucket_from_unix(ev_ts.ravel()).reshape(B, max_len)
        ev_d = _dow_from_unix(ev_ts.ravel()).reshape(B, max_len)
        ev_m = _month_from_unix(ev_ts.ravel()).reshape(B, max_len)

        hour_count = ((ev_h == s_hb.reshape(-1, 1)) & valid).sum(axis=1).astype(np.float32)
        dow_count = ((ev_d == s_dw.reshape(-1, 1)) & valid).sum(axis=1).astype(np.float32)
        month_count = ((ev_m == s_mo.reshape(-1, 1)) & valid).sum(axis=1).astype(np.float32)
        out[f"user_dense_feats_{fid_hour}"] = hour_count
        out[f"user_dense_feats_{fid_dow}"] = dow_count
        out[f"user_dense_feats_{fid_month}"] = month_count
    return out
...
def _compute_raw_features(
    ...
    compute_temporal_features: bool = False,
    domain_ts_cols: Optional[Dict[str, Optional[str]]] = None,
    temporal_max_len: int = 1024,
) -> Dict[str, np.ndarray]:
    names = batch.schema.names
    idx = {name: i for i, name in enumerate(names)}
    B = batch.num_rows
    ...
    # 11thMay-PlanB: temporal bucket features (12 dense, vectorized numpy)
    if compute_temporal_features and domain_ts_cols is not None:
        raw_counts = _compute_temporal_bucket_features(
            batch, domain_ts_cols, max_len=temporal_max_len
        )
        for name, vals in raw_counts.items():
...
def fit_stats(
    ...
    state = PrefixState()
    compute_match = feature_set not in ("fe01a", "11thmay_a")
    compute_temporal = feature_set == "11thmay_a"
    for _, batch in iter_batches(row_groups, batch_size):
        feats = _compute_raw_features(
            ...
            compute_match,
            compute_temporal_features=compute_temporal,
            domain_ts_cols=domain_ts_cols,
...
def build_alignment(
    ...
    if feature_set == "fe01a":
        ...
        # Add a mapping list entry for each of the 12 temporal bucket fids so
        # downstream audits can trace them back to PlanB. (These were dropped by
        # the FE-01 docx filter above because the base list only has FE-01 refs.)
        for fid in range(113, 125):
            d_idx = (fid - 113) % 4
            kind_idx = (fid - 113) // 4
            kind = ("hour_bucket", "dow", "month")[kind_idx]
            domain = ("domain_a", "domain_b", "domain_c", "domain_d")[d_idx]
            mappings.append({
                "docx_ref": "11thMay-PlanB",
                "docx_text": f"user_dense_feats_{fid} = log1p(count_{kind}_match({domain}))",
                "implementation": (
                    f"Vectorized numpy: count of {domain} historical events whose "
                    f"{kind}(event_ts) == {kind}(sample_ts), event_ts<=sample_ts. "
                    "UTC timezone. log1p + train-row-group z-score."
                ),
                "output_column": f"user_dense_feats_{fid}",
            })
    elif feature_set == "fe01b":
        not_included.extend([
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=1024,
        help="11thMay-PlanB: max sequence length scanned per domain for "
             "temporal-bucket match counts. Should be >= the largest "
             "--seq_max_lens used in train.py.",
    )
    args = ap.parse_args()
    ...
    if args.feature_set in ("fe01a", "11thmay_a"):
        match_col, match_ts_col = "", ""
    else:
        match_col, match_ts_col = resolve_domain_d_columns(schema, first_names)
    # 11thMay-PlanB: resolve all 4 domain ts columns up front
    if args.feature_set == "11thmay_a":
        domain_ts_cols = resolve_domain_ts_columns(schema, first_names)
    else:
        domain_ts_cols = None
    ...
    if domain_ts_cols is not None:
        print(f"11thMay-PlanB domain ts columns: {domain_ts_cols}")
    ...
    stats = fit_stats(
        ...
        domain_ts_cols=domain_ts_cols,
        temporal_max_len=args.temporal_max_len,
    )
    ...
    _write_json(
        os.path.join(args.output_dir, "feature_engineering_stats.json"),
        {
            ...
            "temporal_domain_ts_cols": domain_ts_cols,
    ...
    if args.feature_set in {"fe01a", "fe01b", "11thmay_a"}:
        _write_json(
