...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            row_group_range: ``(start, end)`` slice of Row Groups; ``None`` to
                use all Row Groups.
            row_group_indices: explicit Row Group indices into the global
                file-order list. Takes precedence over row_group_range.
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        # ---- item_dense: [[fid, dim], ...] (optional; absent in the original
        # baseline schema, enabled by feature-engineering datasets) ----
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    rg_order = list(range(total_rgs))
    if split_by_timestamp:
        timed_order = []
        for rg_pos, (f, rg_idx, _) in enumerate(rg_info):
            pf = pq.ParquetFile(f)
            schema_names = pf.schema_arrow.names
            if 'timestamp' not in schema_names:
                raise KeyError("--split_by_timestamp requires a timestamp column")
            ts_idx = schema_names.index('timestamp')
            max_ts = None
            for batch in pf.iter_batches(columns=['timestamp'], row_groups=[rg_idx]):
                arr = batch.column(0).fill_null(0).to_numpy(zero_copy_only=False)
                if len(arr) > 0:
                    cur = int(arr.max())
                    max_ts = cur if max_ts is None else max(max_ts, cur)
            timed_order.append((max_ts if max_ts is not None else 0, rg_pos))
        rg_order = [pos for _, pos in sorted(timed_order)]
        logging.info("Using timestamp-ordered row-group split for validation")
    ...
    train_indices = rg_order[:n_train_rgs]
    valid_indices = rg_order[n_train_rgs:]
